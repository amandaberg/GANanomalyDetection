...
class Network:
			...
			with absolute_name_scope(self.scope): # ignore surrounding name_scope
				with tf.control_dependencies(None): # ignore surrounding control_dependencies
					if self.name == 'G_anomaly':
						...
						if "images_in" in self.static_kwargs:
							self.input_templates = [self.static_kwargs["images_in"]]
							self.static_kwargs.pop("images_in")
						else:
	...
	def find_var(self, var_or_localname,extra_scope=False):
		assert is_tf_expression(var_or_localname) or isinstance(var_or_localname, str)
		#print(var_or_localname)
		if extra_scope:
			prefix = self.scope+'/'
			if self.scope == var_or_localname[0:len(self.scope)]:
				var_or_localname = var_or_localname[len(prefix):]
		if not var_or_localname == 'ano_z':
			if var_or_localname in self.vars:
	...
	def copy_vars_from(self, src_net, extra_scope=False):
...
class AnomalyDetectorEncoder(object):
	def __init__(self, config, Ga, Da_Gout, Da_test, Ea, test_data_folder, ano_para=0.1, test_batch_size=8,dis_loss='feature'):
