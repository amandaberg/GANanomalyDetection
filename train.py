...
def setup_snapshot_image_grid(G, training_set,
	...
	# Generate latents.
	latents = misc.random_latents_one_hot(gw * gh, G,layout=layout)
...
def train_progressive_gan(
	...
	for gpu in range(config.num_gpus):
		with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
			...
			with tf.name_scope('E_loss'), tf.control_dependencies(lod_assign_ops):
				E_loss = tfutil.call_func_by_name(G=G_gpu, E=E_gpu, opt=E_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu,	**config.E_loss)
	...
	latents_out = E.run(grid_fakes, minibatch_size=sched.minibatch//config.num_gpus);	
	grid_fakes_reconstructed = Gs.run(latents_out, grid_labels, minibatch_size=sched.minibatch//config.num_gpus);
	...
	misc.save_image_grid(grid_fakes_reconstructed, os.path.join(result_subdir, 'reconstructed_fakes%06d.png' % 0), drange=drange_net, grid_size=grid_size)
	...
	while cur_nimg < total_kimg * 1000:
		...
		for repeat in range(minibatch_repeats):
			...
			tfutil.run([E_train_op], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch})
		...
		if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
			...
			if cur_tick % image_snapshot_ticks == 0 or done:
				...
				misc.save_image_grid(grid_fakes_reconstructed, os.path.join(result_subdir, 'fakes_reconstructed%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
