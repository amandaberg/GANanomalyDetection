...
def create_mnist_anomaly_detection(tfrecord_dir, mnist_dir, percentage, id):
	print('Loading MNIST from "%s"' % mnist_dir)
	import gzip
	with gzip.open(os.path.join(mnist_dir, 'train-images-idx3-ubyte.gz'), 'rb') as file:
		images = np.frombuffer(file.read(), np.uint8, offset=16)
	...
	for i,ii in enumerate(I_normal):
		...
		while len(filename) < 10:
			filename = '0' + filename
		cv2.imwrite(os.path.join(tfrecord_dir,'validation','cell',filename),images[ii,0,:,:])
...
def export_mnist_anomaly_detection_validation_samples(destination_dir, mnist_dir):
	...
	for i,ii in enumerate(I_normal):
		...
		while len(filename) < 10:
			filename = '0' + filename
		cv2.imwrite(os.path.join(destination_dir,'normal',filename),images[ii,0,:,:])
...
def create_cifar10(tfrecord_dir, cifar10_dir,percentage):
...
def create_cifar10_anomaly_detection(tfrecord_dir, cifar10_dir, anomaly_class, percentage):
	...
	for i,ii in enumerate(I):
		...
		while len(filename) < 10:
			filename = '0' + filename
		cv2.imwrite(os.path.join(tfrecord_dir,"train",filename),cv2.cvtColor(np.moveaxis(images[ii,:,:,:],0,-1),cv2.COLOR_RGB2BGR))
		#im_rgb = images[ii,:,:,:];
		#im_gray = 0.2989*im_rgb[0,:,:] + 0.5870*im_rgb[1,:,:] + 0.1140*im_rgb[2,:,:];
			...
			with lmdb.open(lmdb_dir, readonly=True).begin(write=False) as txn:
				for idx, (key, value) in enumerate(txn.cursor()):
					try:
						...
						while len(filename) < 10:
							filename = '0' + filename
						#cv2.imwrite(os.path.join(tfrecord_dir,"train",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2BGR))
						#cv2.imwrite(os.path.join(tfrecord_dir,"train",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2GRAY))
					except:
						print(sys.exc_info()[1])
					if tfr.cur_images == max_images:
						break
...
#	os.mkdir(os.path.join(tfrecord_dir,"validation","cell"))
#	for lsun_class in use_these_classes_for_training:
#		lmdb_dir = os.path.join(lsun_dir,lsun_class + '_val_lmdb');
#		with lmdb.open(lmdb_dir, readonly=True).begin(write=False) as txn:
#			for idx, (key, value) in enumerate(txn.cursor()):
#				try:
#					try:
...
#					img = np.asarray(img)
#					img = img.transpose(2, 0, 1) # HWC => CHW
#					filename = str(idx)+'_'+lsun_class+'.png'
#					while len(filename) < 10 + len(lsun_class) + 1:
#						filename = '0' + filename
#					#cv2.imwrite(os.path.join(tfrecord_dir,"validation","cell",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2BGR))
#					cv2.imwrite(os.path.join(tfrecord_dir,"validation","cell",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2GRAY))
#				except:
#					print(sys.exc_info()[1])
#				#if idx >= 1000:
#				#	break
...
#					img = np.asarray(img)
#					img = img.transpose(2, 0, 1) # HWC => CHW
#					filename = str(i)+'_'+lsun_class+'.png'
#					while len(filename) < 10 + len(lsun_class) + 1:
#						filename = '0' + filename
#					i = i+1
#					#cv2.imwrite(os.path.join(tfrecord_dir,"validation","junk",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2BGR))
#					cv2.imwrite(os.path.join(tfrecord_dir,"validation","junk",filename),cv2.cvtColor(np.moveaxis(img,0,-1),cv2.COLOR_RGB2GRAY))
#				except:
#					print(sys.exc_info()[1])
...
def create_from_images_cellvideos(tfrecord_dir, image_dir, shuffle):
	print('Loading images from "%s"' % image_dir)
	image_filenames = sorted(glob.glob(os.path.join(image_dir, '*.png')))
	if len(image_filenames) == 0:
		error('No input images found')
	...
	with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
		...
		for idx in range(order.size):
			...
			if img.shape[0] == 63:
				img = np.concatenate((img,np.expand_dims(img[img.shape[0]-1,:,:],axis=0)),axis=0)
			if img.shape[1] == 63:
				img = np.concatenate((img,np.expand_dims(img[:,img.shape[1]-1,:],axis=1)),axis=1)
			#-------
...
def create_from_images_cellvideos_ratio(tfrecord_dir, image_dir, included_anomaly_dir, shuffle):
	print('Loading images from "%s"' % image_dir)
	anomaly_filenames = sorted(glob.glob(os.path.join(included_anomaly_dir, '*.png')))
	image_filenames = sorted(glob.glob(os.path.join(image_dir, '*.png')))
...
def execute_cmdline(argv):
	...
	p = add_command(    'create_mnist_anomaly_detection',     'Create dataset for MNIST.',
											'create_mnist datasets/mnist ~/downloads/mnist')
	p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
	p.add_argument(     'mnist_dir',        help='Directory containing MNIST')
	...
	p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
	p.add_argument(     'image_dir',        help='Directory containing the images')
	p.add_argument(     'included_anomaly_dir',        help='Directory containing list of anomalies that should be included in the training data')
	p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
