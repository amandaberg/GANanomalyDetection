...
#----------------------------------------------------------------------------
# Generate a test image based on a random z and save z.
...
def generate_test_image_with_corresponding_z(run_id, snapshot=None, grid_size=[1,1], 
	...
	for idx, latent in enumerate(all_latents):
		...
		images = Gs.run(latent, labels, minibatch_size=minibatch_size, num_gpus=config.num_gpus, out_mul=127.5, out_add=127.5, out_shrink=image_shrink, out_dtype=np.uint8)
		grid = misc.create_image_grid(images, grid_size).transpose(1, 2, 0) # HWC
		if image_zoom > 1:
			grid = scipy.ndimage.zoom(grid, [image_zoom, image_zoom, 1], order=0)
		if grid.shape[2] == 1:
...
def anomaly_detection_encoder(run_id, log, test_data_folder, test_batch_size=64, n_samples=1000):
